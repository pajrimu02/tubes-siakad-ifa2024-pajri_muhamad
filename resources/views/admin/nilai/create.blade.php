    ...
    <div class="page-header">
        <div class="header-left">
            ...
            <div class="header-title">
                <h1>Tambah Nilai</h1>
                <p>Isi form untuk menambahkan nilai mahasiswa</p>
            </div>
        </div>
    </div>
    ...
    <div class="form-card">
        <form action="{{ route('nilai.store') }}" method="POST">
            ...
            <div class="form-grid">
                ...
                {{-- Grade --}}
                <div class="form-group">
                    <label for="nilai">Grade <span class="required">*</span></label>
                    <select id="nilai" name="nilai"
                            class="form-control @error('nilai') is-invalid @enderror" required
                            onchange="autoAngka(this.value)">
                        <option value="" disabled selected>-- Pilih Grade --</option>
                        <option value="A" {{ old('nilai') == 'A' ? 'selected' : '' }}>A — Sangat Baik (≥ 85)</option>
                        <option value="B" {{ old('nilai') == 'B' ? 'selected' : '' }}>B — Baik (70–84)</option>
                        <option value="C" {{ old('nilai') == 'C' ? 'selected' : '' }}>C — Cukup (55–69)</option>
                        <option value="D" {{ old('nilai') == 'D' ? 'selected' : '' }}>D — Kurang (40–54)</option>
                        <option value="E" {{ old('nilai') == 'E' ? 'selected' : '' }}>E — Tidak Lulus (&lt; 40)</option>
                    </select>
...
@push('scripts')
<script>
function autoAngka(grade) {
    const map = { A: 90, B: 75, C: 62, D: 47, E: 30 };
    const angka = document.getElementById('angka');
    if (!angka.value) angka.value = map[grade] ?? '';
}
</script>
@endpush