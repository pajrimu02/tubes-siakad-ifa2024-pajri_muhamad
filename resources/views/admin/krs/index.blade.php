...
<style>
    ...
    /* ── CARD ── */
    .krs-card { border-radius: 16px; border: 1px solid #f1f5f9; box-shadow: 0 2px 12px rgba(0,0,0,0.06); overflow: hidden; }
    .krs-card .card-header { background: #fff; border-bottom: 1px solid #f1f5f9; padding: 15px 20px; display: flex; align-items: center; justify-content: space-between; }
    .krs-card .card-header h5 { font-size: 15px; font-weight: 700; color: #1e293b; margin: 0; display: flex; align-items: center; gap: 8px; }
    ...
    /* ── BADGES ── */
    .badge-ganjil { display: inline-flex; align-items: center; padding: 3px 10px; border-radius: 999px; font-size: 12px; font-weight: 600; background: #ede9fe; color: #5b21b6; }
    .badge-genap  { display: inline-flex; align-items: center; padding: 3px 10px; border-radius: 999px; font-size: 12px; font-weight: 600; background: #dcfce7; color: #166534; }
    .badge-tahun  { display: inline-flex; align-items: center; padding: 3px 10px; border-radius: 999px; font-size: 12px; font-weight: 600; background: #e0f2fe; color: #0369a1; }
    ...
    /* ── AVATAR ── */
    .mhs-avatar { width: 30px; height: 30px; border-radius: 50%; background: linear-gradient(135deg,#6366f1,#4f46e5); color: #fff; font-size: 12px; font-weight: 700; display: inline-flex; align-items: center; justify-content: center; margin-right: 8px; flex-shrink: 0; }
...
<div class="container-fluid">
    ...
    <div class="row mb-3 align-items-center">
        ...
        <div class="col-md-6">
            <div class="d-flex gap-2 flex-wrap">
                ...
                <a href="{{ route('krs.export', ['tipe' => request('tipe')]) }}"
                   class="btn-modern btn-export">
                    <i class="fa fa-file-excel"></i> Export Excel
                </a>

                {{-- IMPORT --}}
                <form action="{{ route('krs.import') }}" method="POST"
        ...
        <div class="col-md-6 d-flex justify-content-end mt-2 mt-md-0">
            {{-- SEARCH dengan form GET --}}
            <form method="GET" action="{{ route('krs.index') }}" class="d-flex gap-2">
                @if(request('tipe'))
                ...
                @if(request('search'))
                <a href="{{ route('krs.index', ['tipe' => request('tipe')]) }}"
    ...
    <div class="filter-bar">
        ...
        <a href="{{ route('krs.index', array_filter(['search' => request('search')])) }}"
           class="filter-chip {{ !request('tipe') ? 'active' : '' }}">
            <span class="chip-dot"></span> Semua
        </a>

        <a href="{{ route('krs.index', array_filter(['tipe' => 'ganjil', 'search' => request('search')])) }}"
           class="filter-chip {{ request('tipe') === 'ganjil' ? 'active' : '' }}">
            <span class="chip-dot"></span>
            <i class="fa fa-sun" style="font-size:11px;"></i> Ganjil (1,3,5,7)
        </a>

        <a href="{{ route('krs.index', array_filter(['tipe' => 'genap', 'search' => request('search')])) }}"
           class="filter-chip {{ request('tipe') === 'genap' ? 'active' : '' }}">
            <span class="chip-dot"></span>
            <i class="fa fa-moon" style="font-size:11px;"></i> Genap (2,4,6,8)
        </a>
    </div>
    ...
    <div class="card krs-card border-0">
        ...
        <div class="card-header">
            <h5>
                ...
                Data KRS
                @if(request('tipe'))
                    <span class="{{ request('tipe') === 'ganjil' ? 'badge-ganjil' : 'badge-genap' }} ms-1">
                        Semester {{ ucfirst(request('tipe')) }}
                    </span>
                @endif
                @if(request('search'))
                    <span class="badge-tahun ms-1">
            ...
            <div class="table-responsive">
                <table class="table table-modern mb-0">
                    ...
                    <tr>
                        ...
                        <td>
                            @php $smt = $k->semester ?? 0; @endphp
                            @if(in_array($smt, [1,3,5,7]))
                                <span class="badge-ganjil">Smt {{ $smt }} · Ganjil</span>
                            @else
                                <span class="badge-genap">Smt {{ $smt }} · Genap</span>
                            @endif
                        </td>
                    ...
                    <tr>
                        <td colspan="7">
                            <div class="empty-state">
                                ...
                                <p>Tidak ada data KRS
                                    @if(request('tipe')) untuk semester {{ request('tipe') }} @endif
